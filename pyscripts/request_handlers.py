...
async def get_state_handler(request: Request, ip_address: str,
                            phone_id: str, device_id: str,
                            device_password: str) -> HTTPResponse:
    """Use for handling GET requests to /switcher/get_state.

    Returns a json object represnting the current state.
    """
    try:
        async with SwitcherV2Api(get_running_loop(), ip_address, phone_id,
                                 device_id, device_password) as swapi:
            response = await swapi.get_state()

        if not response:
            raise ServiceUnavailable(
                "Failed to get response from api.", 503)
...
async def turn_off_handler(request: Request, ip_address: str,
                           phone_id: str, device_id: str,
                           device_password: str) -> HTTPResponse:
    ...
    try:
        ...
        if response and response.msg_type == \
                messages.ResponseMessageType.CONTROL:
...
async def get_schedules_handler(request: Request, ip_address: str,
                                phone_id: str, device_id: str,
                                device_password: str) -> HTTPResponse:
            ...
            if response.found_schedules:
                for schedule in response.get_schedules:
                    ...
                    schedules_list.append({
                        ...
                        consts.KEY_SCHEDULE_DATA: schedule_obj.schedule_data,
...
async def turn_on_handler(request: Request, ip_address: str,
                          phone_id: str, device_id: str,
                          device_password: str) -> HTTPResponse:
...
async def set_auto_shutdown_handler(request: Request, ip_address: str,
                                    phone_id: str, device_id: str,
                                    device_password: str) -> HTTPResponse:
...
async def set_device_name_handler(request: Request, ip_address: str,
                                  phone_id: str, device_id: str,
                                  device_password: str) -> HTTPResponse:
    ...
    try:
        ...
        if response and response.msg_type == \
                messages.ResponseMessageType.UPDATE_NAME:
            return json({consts.KEY_SUCCESSFUL: response.successful})
        return json({
            consts.KEY_SUCCESSFUL: False,
...
async def enable_schedule_handler(request: Request, ip_address: str,
                                  phone_id: str, device_id: str,
                                  device_password: str) -> HTTPResponse:
    ...
    try:
        ...
        async with SwitcherV2Api(get_running_loop(), ip_address, phone_id,
                                 device_id, device_password) as swapi:
            response = \
                await swapi.disable_enable_schedule(updated_schedule_data)
...
async def disable_schedule_handler(request: Request, ip_address: str,
                                   phone_id: str, device_id: str,
                                   device_password: str) -> HTTPResponse:
...
async def delete_schedule_handler(request: Request, ip_address: str,
                                  phone_id: str, device_id: str,
                                  device_password: str) -> HTTPResponse:
    ...
    try:
        ...
        async with SwitcherV2Api(get_running_loop(), ip_address, phone_id,
                                 device_id, device_password) as swapi:
            response = \
                await swapi.delete_schedule(schedule_id)
...
async def _create_raw_schedule_data(
        schedule_days: List[int], start_hours: int, start_minutes: int,
        stop_hours: int, stop_minutes: int) -> str:
    """Use for creating raw schedule data for creating schedule request."""
    running_loop = get_running_loop()

    weekdays = await create_weekdays_value(running_loop,
                                           schedule_days)
    start_time = await timedelta_str_to_schedule_time(
        running_loop,
        str(timedelta(hours=start_hours, minutes=start_minutes)))
    end_time = await timedelta_str_to_schedule_time(
        running_loop,
        str(timedelta(hours=stop_hours, minutes=stop_minutes)))
    return SCHEDULE_CREATE_DATA_FORMAT.format(weekdays, start_time, end_time)


def _validate_time_integers(start_hours: int, start_minutes: int,
                            stop_hours: int, stop_minutes: int) -> None:
    """Use to validate start and end time values for creating schedule."""
    if start_hours < 0 or start_hours > 23:
        raise InvalidUsage(
            "Unknown start_hours, accepts 0 to 23.", 400)
    if start_minutes < 0 or start_minutes > 59:
        raise InvalidUsage(
            "Unknown start_minutes, accepts 0 to 59.", 400)
    if stop_hours < 0 or stop_hours > 23:
        raise InvalidUsage(
            "Unknown stop_hours, accepts 0 to 23.", 400)
    if stop_minutes < 0 or stop_minutes > 59:
        raise InvalidUsage(
            "Unknown stop_minutes, accepts 0 to 59.", 400)
...
async def _parse_schedule_body(body: Dict) -> str:
    ...
    _validate_time_integers(start_hours, start_minutes, stop_hours,
                            stop_minutes)
    ...
    return await _create_raw_schedule_data(schedule_days, start_hours,
                                           start_minutes, stop_hours,
                                           stop_minutes)


async def create_schedule_handler(request: Request, ip_address: str,
                                  phone_id: str, device_id: str,
                                  device_password: str) -> HTTPResponse:
    ...
    try:
        ...
        async with SwitcherV2Api(get_running_loop(), ip_address, phone_id,
                                 device_id, device_password) as swapi:
            response = \
                await swapi.create_schedule(schedule_data)
